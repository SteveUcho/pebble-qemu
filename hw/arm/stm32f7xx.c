/*
 * STM32F7xx Microcontroller
 *
 * Copyright (C) 2010 Andre Beckus
 * Copyright (C) 2016 Pebble
 *
 * This implementation contains support for many of the STM32F7xx peripherals based on RM0410 rev 2,
 * but currently uses a cortex-m4 core.
 *
...
static const char *stm32f7xx_periph_name_arr[] = {
    ...
    ENUM_STRING(STM32_LPTIM1),

    ENUM_STRING(STM32_PERIPH_COUNT)
};

/* Init STM32F7XX CPU and memory.
...
void stm32f7xx_init(
            ...
            Stm32F7xxLPTimer **stm32_lptimer,
            ...
            ARMCPU **cpu)
{
    ...
    if (kernel_filename) {
        memory_region_init_ram(flash, NULL, "stm32f7xx.flash", flash_size,
                           &error_abort);
    ...
    if (sram_size) {
        memory_region_init_ram(sram, NULL, "stm32f7xx.sram", sram_size, &error_abort);
    ...
    if (dinfo) {
        f2xx_flash_register(blk_by_legacy_dinfo(dinfo), STM32_FLASH_ADDR_START, flash_size * 1024);
    }
    ...
    for (i = 0; i < ARRAY_LENGTH(uart_desc); ++i) {
        assert(i < STM32F7XX_UART_COUNT);
        const stm32_periph_t periph = STM32_UART1 + i;
        DeviceState *uart_dev = qdev_create(NULL, "stm32f7xx-uart");
        uart_dev->id = stm32f7xx_periph_name_arr[periph];
        ...
        stm32_uart[i] = (Stm32F7xxUart *)uart_dev;
    }
...
#define dummy_dev(name, start, size) do {\
    ...
    *stm32_lptimer = (Stm32F7xxLPTimer *)lptimer;
    ...
    sysbus_connect_irq(SYS_BUS_DEVICE(i2c4), 1, qdev_get_gpio_in(nvic, STM32_I2C4_ER_IRQ));
